...
| **TypeScript** | Structural, gradual (any/unknown escape hatches), erased at runtime | Shape at compile time only. Types disappear at runtime — `any` and type assertions bypass all checks silently. | **Everything at trust boundaries** — external input, API responses, DB results. The type system provides zero runtime guarantees. |
