...
then reset the branch narrative to the mainline form, so the merge does not pollute
the mainline note. If you ever find the default branch's .tsugu/context.md already
carrying a landed branch's narrative (duplicate "##" section headers), collapse it the
same way — but that edits the DEFAULT branch (public coordination), so get approval
from the human before you commit it.
