---
description: Set up or migrate the repo's .tsugu/ workspace + policy.md — asks the minimum; idempotent; re-running migrates older schemas (1→2→3→4→5) (tsugu-schema)
---
...
Load-bearing invariants the skill enforces: stamps `tsugu-schema: 5`; the
...
migration applies steps in order (1→2→3→4→5, stamp written last; the 3→4
