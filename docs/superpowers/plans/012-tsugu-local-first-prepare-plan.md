...
need 'push-prepare-branches'                          "push policy field referenced"
...
need 'human-confirmed|human confirm'                  "cleanup is human-confirmed"
...
# the disposition must NEVER auto-delete (the load-bearing safety fix):
refute 'auto-delete[d]?( the| a)? local|local ref .*auto-delet|automatically delet' "no auto-delete of the local prepare ref"
```
