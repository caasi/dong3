...
| File | Change |
| --- | --- |
| `plugins/tsugu/skills/tsugu/SKILL.md` | step 8 → local-first (push default `no`, opt-in framing) **+ the schema-aware read** (absent → `yes` if schema 4 else `no`); soften the flat "Read the queue from remote-tracking refs" → **local + remote** work prefixes; partition gains the **containment takeover** derivation (a distinct row from 011's *decided*); new **taken-over** disposition (**suppress-and-surface + human-confirmed cleanup, never auto-delete**); the **auto-push invariant** (D); schema `4 → 5`; *Multi-agent* gains the cross-machine push opt-in/exception; B3 note (no remote `prepare/<slug>` under local-first; full B3 survives for opt-in repos) |
