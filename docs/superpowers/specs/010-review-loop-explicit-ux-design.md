...
- **Push only a branch that already has an upstream.** The loop also supports **local-only** targets. Decide by whether the branch has a **branch-specific upstream** — `git rev-parse --abbrev-ref --symbolic-full-name @{upstream}` succeeds — **not** merely whether a remote exists (a remote can exist while *this* branch was never published). If it has an upstream, force-push **explicitly to that upstream ref** — derive `<remote>` and `<upstream-branch>` from `@{upstream}` and run `git push --force-with-lease <remote> HEAD:refs/heads/<upstream-branch>` (the branch may track a differently-named remote branch, so don't assume the local name; and don't rely on a bare `git push`, which depends on `push.default`). If there is no branch upstream, group the history **locally and do not push or publish it** — accepting "group commits" must never turn a private branch into a published one.
