...
8. **abort/rollback on any failure before the push** (a stage/commit error, tree-hash mismatch, failing tests, or a failed stash restore): `git reset --hard <backup>` to restore the original history, restore the stash if not already restored, and **do not push**. Keep `<backup>` until a verified success (then prefer leaving it for the user to prune).
