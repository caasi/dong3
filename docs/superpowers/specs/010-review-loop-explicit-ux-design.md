...
- **Feature branches only.** Never offer (and never perform) a rebase on the **default/primary branch**. Detect it robustly across the **configured remote** (the remote the branch tracks, or the sole/relevant remote `<r>`, not a hardcoded `origin`): `git symbolic-ref --short refs/remotes/<r>/HEAD` **with the `<r>/` prefix stripped** (it returns e.g. `upstream/trunk`, not `trunk`, so compare like-for-like against the current branch name), falling back to the local `main`/`master`/`develop` set and the project's stated default — **not** a hardcoded triple (repos whose default is `trunk`/`production`/etc., possibly on a non-`origin` remote, must be protected too). If detection is inconclusive and the branch isn't clearly a feature branch, **err toward not offering**. If the current branch is the default/primary, skip the offer entirely.
...
- **Push only a branch that already has an upstream.** The loop also supports **local-only** targets. Decide by whether the branch has a **branch-specific upstream** — `git rev-parse --abbrev-ref --symbolic-full-name @{upstream}` succeeds — **not** merely whether a remote exists (a remote can exist while *this* branch was never published). If it has an upstream, force-push **explicitly and safely to that upstream ref**: obtain the remote and remote ref from the branch's upstream config — git's `%(upstream:remotename)` and `%(upstream:remoteref)` atoms (robust even under custom fetch refspecs; don't string-strip the local remote-tracking name) — **capture the upstream SHA before rewriting**, and push with a **pinned lease**: `git push --force-with-lease=<remoteref>:<captured-sha> <remotename> HEAD:<remoteref>`. The pinned lease aborts if the remote moved during the review (a bare `--force-with-lease` would treat a refreshed tracking ref as the expected value and could clobber concurrent commits); never a bare `git push` (depends on `push.default`). **Invariant (the load-bearing rule, exact plumbing left to implementation):** never overwrite remote commits the regrouped branch doesn't contain. On any lease mismatch, **abort and surface to the user** — do not re-capture a possibly-concurrent tip and retry. The human-present convergence and the `<backup>` branch are the ultimate backstop; a concurrent force-push race on a personal feature branch is an extreme edge case, and the invariant + abort-and-surface behavior is what the implementation must guarantee, not a specific capture sequence. If there is no branch upstream, group the history **locally and do not push or publish it** — accepting "group commits" must never turn a private branch into a published one.
...
### The offer
> "The loop added N commits to `<branch>` (incl. M review fixups). Want me to group them before you merge? Feature branch only — I'll keep a backup branch; **if the branch has an upstream I'll force-push to it with `--force-with-lease`, otherwise I'll group it locally (no push)**."

(Upstream-aware wording — don't promise a push on a branch with no upstream.)
...
5. **run the project's tests** if present (Guardrails), on the clean regrouped state;
6. **restore the stash** (if one was taken in step 0) — *before* the push, so a restoration failure (e.g. a colliding untracked file) still rolls back cleanly without rewritten history having been published;
7. **push last** per the local-only/remote rule (Guardrails) — the only externally-visible step, performed only after everything above succeeded;
8. **abort/rollback on any failure before the push** (a stage/commit error, tree-hash mismatch, failing tests, or a failed stash restore): `git reset --hard <backup>` to restore the original history, then restore the stash if not already restored. If the restore is blocked by an **operation-created** untracked artifact (e.g. a test output that collides with a stashed path), remove that operation artifact — never the user's stashed content — and retry. If the stash still cannot be restored cleanly, **STOP and surface to the user** the `<backup>` branch and the `git stash` entry for manual recovery; never force-delete the user's files to force a restore. **Do not push.** Keep `<backup>` until a verified success (then prefer leaving it for the user to prune).
...
- [ ] Accepting groups the commits, keeps a backup branch, and verifies an identical tree; **if the branch has an upstream** it force-pushes to that upstream ref with `--force-with-lease`, and a branch with **no upstream** is grouped locally and ends **without a push**. Declining leaves history untouched.
