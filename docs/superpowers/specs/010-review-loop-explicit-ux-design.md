...
- **Push only a branch that already has an upstream.** The loop also supports **local-only** targets. Decide by whether the branch has a **branch-specific upstream** — `git rev-parse --abbrev-ref --symbolic-full-name @{upstream}` succeeds — **not** merely whether a remote exists (a remote can exist while *this* branch was never published). If it has an upstream, force-push **explicitly to that ref**: `git push --force-with-lease <remote> <branch>` (a bare `git push --force-with-lease` depends on `push.default` and may target an unintended ref). If there is no branch upstream, group the history **locally and do not push or publish it** — accepting "group commits" must never turn a private branch into a published one.
