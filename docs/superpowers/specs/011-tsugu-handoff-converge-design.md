...
| `plugins/tsugu/skills/tsugu/references/policy-and-intake.md` | the `## Housekeeping` field section: keep `stale-after`, but reframe its **consumers** — `prune` surfaces stale in-progress read-only + `converge` shows a stale candidate flag; **no converge "housekeeping cleanup" block** |
| `plugins/tsugu/skills/tsugu/templates/policy.md` | `## Housekeeping` (`stale-after`) field **stays**; update any inline comment that frames it as converge-driven cleanup to "consumed by `prune` + converge's stale flag" |
| `plugins/tsugu/skills/tsugu/README.md` | user-facing doc: three routines → four (add `prune`); converge = handoff (not "completes/lands work"); list `/tsugu:prune` |
